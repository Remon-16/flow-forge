...
- **LLM 输出数量校验（反幻觉）**：所有 LLM 调用在骨架生成、数据填充、断言生成和 URL 纠错后，自动校验输出条目数与输入是否一致。数量不匹配时自动重试。每个校验项支持三级策略（终止 / 警告继续 / 跳过），可通过 `validation.rules` 配置。骨架生成采用分批机制（默认每批 30 个测试点），提高大批量测试计划的计数精度。
