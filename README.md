...
- **分批大小参考**：
  - 强模型：骨架分批建议 30（`skeleton_batch_size`），计划分块建议 8-12（`plan_chunk_size`）
  - 弱模型：骨架分批建议 5-10，计划分块建议 3-5
...
- **骨架分批与计划分块**：骨架生成采用分批机制（默认每批 30 个测试点），提高计数精度。测试计划生成采用"轮廓 + 分块"两步法——先生成轻量 JSON 轮廓，再基于轮廓分块生成完整计划（默认每块 8 个接口），避免大项目下输出截断。
