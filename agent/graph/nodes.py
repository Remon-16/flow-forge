...
# =========================================================================
# Node: validate_interface_urls
# =========================================================================
def validate_interface_urls_node(state: GraphState) -> GraphState:
    """Validate that each interface URL exists in the API document text.

    Runs after analyze_api and before save_interfaces.  For URLs that cannot
    be found in the original API doc, the LLM is asked to correct them (up to
    3 retries).  Interfaces that still fail after retries are kept but marked
    with ``[URL_MAY_INCORRECT]`` in their remark field.
    """
    from agents.api_analyzer import ApiAnalyzer
    ...
    # Step 1: find interfaces whose URL is not in the API doc text
    ...
    # Step 2: attempt LLM correction with retries
    ...
    for iface in bad_interfaces:
        ...
        # Search relevant snippets from API doc using fuzzy matching
        snippets = api_analyzer._fuzzy_search_api_doc(
            url=bad_url,
            http_method=http_method,
            api_doc_text=api_raw_text,
...
def save_interfaces_node(state: GraphState) -> GraphState:
    ...
    Always saves as YAML regardless of output format.  Checks remark fields
    for ``[URL_MAY_INCORRECT]`` markers and prints a summary so the user can
    review and manually fix the YAML files before approving the plan.
    """
    from pathlib import Path
    ...
    for iface in interfaces:
        try:
            ...
            count += 1
            # Check for URL_MAY_INCORRECT marker
...
def reload_interfaces_node(state: GraphState) -> GraphState:
    ...
    This picks up any manual fixes the user made to the YAML files during
    the plan review phase.  Performs a quick substring check on reloaded
    URLs against the API doc text and warns if any still do not match.
    """
    state.setdefault("errors", [])
    ...
    if _sl():
        _sl().log_event("reload_interfaces", count=len(reloaded),
                         url_issues=len(url_issues))
        _sl().log_node_end("reload_interfaces")
