...
def write_output_node(state: GraphState) -> GraphState:
    ...
    logger.info(_step("write_output", "pipeline.write_output"))
    if _sl():
    ...
    if output_format in ("excel", "both"):
        try:
            ...
            logger.info(_("output.excel_written", path=output_path))
            if _sl():
    ...
    if output_format in ("yaml", "both"):
        logger.info(_("output.yaml_saved", dir=cases_dir, iface_count=_count_yaml(cases_dir, "interfaces"), single_count=_count_yaml(cases_dir, "single_cases"), biz_count=_count_yaml(cases_dir, "biz_flows")))

    if failures:
        logger.info(_("output.validation_failed", count=len(failures), dir=cases_dir))
