...
9. **Plan Parsing**: Reads pre-split section data from `plan_sections.json` and parses it into a structured TestPlan using token-aware greedy chunking (whole → case_type split → greedy per-section). No longer parses `plan.md`. After parsing, runs `flow_match` validation — matching LLM-generated Mermaid flow diagrams against business scenarios defined in the requirement document; mismatched flows are handled per the `parse_plan_validation` configuration (retry correction / discard / keep with warning). See [anti-hallucination.md](./anti-hallucination.en.md#flow-association-validation-flow_match).
...
| **parse_plan** (plan parsing) | plan_sections.json structural split + greedy algorithm | Dedup by `test_id` + `url`; after parsing, runs `flow_match` validation matching Mermaid diagrams against requirement scenarios, handling mismatches per `parse_plan_validation` strategy | Reads pre-split sections from `plan_sections.json`, 3-level strategy: whole → case_type split (`single_api` / `biz_flows`) → greedy per-section accumulation, each batch within token budget |
