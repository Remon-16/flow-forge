...
**作用**：计划解析完成后，系统校验 LLM 输出的业务场景是否与需求分析阶段提取的 Mermaid 流程图正确关联。若存在无法匹配到任何流程图的"孤立场景"，说明 LLM 可能产生了幻觉（编造了需求中不存在的业务流程）。

**处理流程（两步走）**：

1. **第一步 — 代码精确名称匹配**：调用 `match_mermaids_to_scenarios()` 将每个场景名称与流程图名称做精确匹配。大多数情况下 LLM 不会修改名称，此步骤可高效匹配。
2. **第二步 — LLM 语义匹配兜底**：若第一步存在失配场景，将孤儿场景与全部 Mermaid 图发送给 LLM 做语义关联（支持多对一：一个 Mermaid 图可关联多个场景，如正常路径 + 异常路径共享同一流程图）。LLM 返回后由代码校验结果有效性（ID 是否存在、场景是否全部覆盖），已成功匹配的场景不参与后续重试。最多重试 `max_retries` 次。
3. 若重试耗尽后仍有失配，按 `strategy` 处理：
   - `fail`：抛出异常，终止流水线
   - `skip`：跳过校验，保留所有流程
   - `warn`：按 `failure_action` 处理（见下）

**failure_action**（仅 `strategy: warn` 时生效）：

| failure_action | 行为 |
|----------------|------|
| `discard`（默认） | 丢弃无法匹配的孤立场景，从计划中移除 |
| `keep` | 保留孤立场景，添加警告标记交由人工复核 |

> **推荐配置**：与 URL 纠错一致，建议 `warn + keep`。LLM 偶尔会产生需求中确实存在但匹配算法无法关联的合法场景（如为同一业务流程生成多个测试场景变体），`keep` 确保不丢失有效测试场景。
