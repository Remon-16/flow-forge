...
![智能体执行器任务配置](./images/studio_agent_workflow_setting.png)
...
![恢复模式回到计划审核节点（未打开批注器侧边栏）](./images/studio_agent_workflow_resume_plan_confirm_node_example.png)

![恢复模式回到计划审核节点（已打开批注器两个侧边栏）](./images/studio_agent_workflow_resume_plan_confirm_node_example2.png)

![恢复模式回到计划审核节点（关闭批注器，用 n 输入审核意见）](./images/studio_agent_workflow_resume_plan_confirm_node_example3_hide_plan.png)

> **说明**：以上三张截图展示「从计划审核节点恢复（resume）」的交互。截图中看到的计划数据来自此前用 Qwen3-8B-Q4_K_M 实际生成后从审核节点恢复的运行结果，仅用于演示界面交互。
...
> **大整数精度注意：** JavaScript Number 类型仅能安全表示 -9,007,199,254,740,991 ~ 9,007,199,254,740,991（约 9×10¹⁵）范围内的整数。如果 `env-*.yml` 中需要配置超过此范围的长整数（如 64 位用户 ID），使用 YAML 字符串语法（加引号）：`id: "1000000000000000001"`。Python 执行器和转换器无此限制，可正确处理任意大小整数。
...
![Excel 编辑器示例](./images/excel_edit_example.png)
...
![YAML 编辑器示例](./images/yaml_edit_example.png)
...
![计划批注器示例](./images/studio_plan_example.png)
